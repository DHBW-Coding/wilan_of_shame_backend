...
class DeviceEvent:
    ...
    def request_device_name(self, lease_file='/var/lib/misc/dnsmasq.leases'):
...
def start_sniffing():
    # Filters
    filter_tcp_out = "tcp and not src net 192.168.4.1/24"
    filter_no_gw = "not src 192.198.4.1"

    # Start sniffing packets on the 'wlp2s0' interface
    sniff(filter=filter_no_gw, prn=packet_callback, store=False, iface='wlan0')
